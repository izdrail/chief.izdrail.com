...
// detectSetupResultMsg is sent when Ollama finishes detecting setup commands.
...
// DisableRetry disables automatic retry on agent errors.
...
func (a App) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case detectSetupResultMsg:
		return a.handleDetectSetupResult(msg)
...
func (a App) startLoopForPRD(prdName string) (tea.Model, tea.Cmd) {
	...
	onCorrectBranch := branch == fmt.Sprintf("chief/%s", prdName)

	if !isProtected && !anotherRunningInSameDir && onCorrectBranch {
		// No conflicts and already on the right branch: skip the dialog entirely and start the loop directly
		return a.doStartLoop(prdName, prdDir)
	}
...
func (a *App) runWorktreeStep(step WorktreeSpinnerStep, baseDir, worktreePath, branchName string) tea.Cmd {
	...
	case SpinnerStepRunSetup:
		...
		if setupCmd == "" {
			// If no setup command is configured, try to detect one
			return a.runDetectSetup()
		}
		return func() tea.Msg {
...
// runDetectSetup uses Ollama to detect a setup command for the worktree.
func (a *App) runDetectSetup() tea.Cmd {
	return func() tea.Msg {
		prompt := embed.GetDetectSetupPrompt()
		client := ollama.NewClient()

		msg, err := client.Chat(context.Background(), ollama.ChatRequest{
			Model: client.Model,
		...
		if err != nil {
			return detectSetupResultMsg{err: fmt.Errorf("Ollama detection failed: %w", err)}
		}

		return detectSetupResultMsg{command: strings.TrimSpace(msg.Content)}
	}
}

// handleDetectSetupResult handles the result of the setup command detection.
func (f App) handleDetectSetupResult(msg detectSetupResultMsg) (tea.Model, tea.Cmd) {
	if msg.err != nil {
		f.worktreeSpinner.SetError(fmt.Sprintf("Failed to detect setup command: %s", msg.err.Error()))
		return f, nil
	}

	// Use detected command
	f.config.Worktree.Setup = msg.command
	return f, f.runWorktreeStep(SpinnerStepRunSetup, f.baseDir, f.pendingWorktreePath, "")
}
