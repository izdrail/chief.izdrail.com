...
// SetStore sets the SQLite store for the manager and its loops.
func (m *Manager) SetStore(s *db.Store) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.store = s
}
...
func (m *Manager) RegisterWithWorktree(name, prdPath, repoURL, worktreeDir, branch string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) Start(name string) error {
	...
	instance.Loop.SetStore(m.store)
	instance.Loop.SetRepoURL(instance.RepoURL)
	m.mu.RUnlock()
...
// Remove stops and completely unregisters a PRD from the manager.
...
func (m *Manager) UpdateWorktreeInfo(name, repoURL, worktreeDir, branch string) error {
	m.mu.RLock()
	instance, exists := m.instances[name]
	m.mu.RUnlock()
