...
//go:embed generate_prd_prompt.txt
var generatePRDPromptTemplate string
...
// GetGeneratePRDPrompt returns the one-shot PRD generation prompt.
func GetGeneratePRDPrompt(prdDir, name, description string) string {
	result := strings.ReplaceAll(generatePRDPromptTemplate, "{{PRD_DIR}}", prdDir)
	result = strings.ReplaceAll(result, "{{NAME}}", name)
	return strings.ReplaceAll(result, "{{DESCRIPTION}}", description)
}
